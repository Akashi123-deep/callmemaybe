...
class ConstructPrompt(BaseModel):
    ...
    def injected_prompt(self) -> str:
        prompt = (
            ...
            '1. Preserve the numeric value exactly: do not round or '
            'truncate, regardless of magnitude (large numbers, decimals, '
            'and negatives). The spelling may be normalized to match the '
            'declared type.\n'
